 ...
 * used to capture audio, and will stop when
 * <code>stopRecording()</code> is called. Calling
 * <code>getData()</code> returns the captured audio data as Data objects. </p>
 * <p/>
...
public class Microphone extends BaseDataProcessor {
     ...
     * @param closeBetweenUtterances whether or not the microphone will release
     * the audio between utterances. On certain systems (Linux for one), closing
     * and reopening the audio does not work too well. The default is false for
     * Linux systems, true for others
     * @param msecPerRead the number of milliseconds of audio data to read each
     * time from the underlying Java Sound audio device.
     * @param keepLastAudio whether to keep the audio data of an utterance
     * around until the next utterance is recorded.
     * @param stereoToMono how to convert stereo audio to mono. Currently, the
     * possible values are "average", which averages the samples from at each
     * channel, or "selectChannel", which chooses audio only from that channel.
     * If you choose "selectChannel", you should also specify which channel to
     * use with the "selectChannel" property.
     * @param selectedChannel the channel to use if the audio is stereo
     * @param selectedMixerIndex the mixer to use. The value can be "default,"
     * (which means let the AudioSystem decide), "last," (which means select the
     * last Mixer supported by the AudioSystem), which appears to be what is
     * often used for USB headsets, or an integer value which represents the
     * index of the Mixer.Info that is returned by AudioSystem.getMixerInfo().
...
//    	    audioLine.flush();
        ...
        private Data readData(Utterance utterance) throws IOException {
            ...
            return (new DoubleData(samples, (int) audioStream.getFormat().getSampleRate(),
     ...
     * <code>startRecording()</code> and
     * <code>stopRecording()</code> is cached in an Utterance object. Calling
     * this method basically returns the next chunk of audio data cached in this
     * Utterance.
     *
     * @return the next Data or
    ...
    /**
     * added by cclo for recording voice
     */
    boolean recWav = false;
    boolean fileOpened = false;


    /*
     * called in first recording
     */
    public void ccloStartRecording() {

        try {
            currentUtterance.ccloBeginRec();
            fileOpened = true;
    ...
    /*
     * stop recording effort
     */
    public void ccloStopRecording() {
        if (!fileOpened) {
            return;
        }
        try {
            currentUtterance.ccloWrieFile();
        } catch (Exception e) {
            e.printStackTrace();
        }
        recWav = false;
    ...
    public boolean ccloSetRecording(boolean val) {
        if (fileOpened) {
            recWav = val;
            return true;
        } else {
