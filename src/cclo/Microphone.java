 ...
 * error, and a call <code>startRecording</code> returns false. </p>
 */
...
//    	    audioLine.flush();
    ...
    public void ccloSaveRecording() {
        try {
            String wavFileName = new SimpleDateFormat("[MM dd]_[HH mm]").format(Calendar.getInstance().getTime());
            wavFileName += ".wav";
            show("Save Utterance, size: " + currentUtterance.getAudioSize());
            currentUtterance.ccloWrieFile(wavFileName);
            currentUtterance = new Utterance("Microphone", audioStream.getFormat());
        } catch (Exception e) {
            e.printStackTrace();
        }
